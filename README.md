# One way to find comfort with the Bell proof and the results of the Bell experiments
...
encountered in a Bell-type experiment. In this approach, the outcomes of a pair of
spin component measurements on two entangled spin 1/2 particles in the singlet state
can be explained as the result of a shared hidden variable. I leave it to others to
decide whether the model is local or non-local. I think it is local, but I know others
disagree.
As an offshoot, the maximum degree to which Bell’s inequality is violated
is shown to be indicative of the number of spatial dimensions.

This program accompanies an article with the title 
"Three ways to find comfort with the Bell proof and the
results of the Bell experiments" by Richard D. Gill, Inge S. Helland, and Bart Jongejan (me).
